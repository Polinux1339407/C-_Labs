...
double amplitude(double* array, size_t size_array) {
    ...
    double average_value = sum / (size_array - 1);

    for (size_t i = 0; i < size_array; i++)
        max_amplitude = std::max(max_amplitude, std::abs(average_value - array[i]));
    return max_amplitude;
}

void generate_sine(double** new_signal, size_t size_array, double freq, double ampl) {
    if(new_signal != nullptr)
        delete[] *new_signal;
    (*new_signal) = new double[size_array];
    for (size_t i = 0; i < size_array; i++) {
        (*new_signal)[i] = ampl * std::sin(2 * M_PI * freq * i / discr_freq);
    }
}
